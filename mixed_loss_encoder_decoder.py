...
Channel_en_shape=np.shape(dataChannel['x'][0])
Channel_de_shape=np.shape(dataChannel['y'][0])
##########################################################################
...
def encoder_decoder():
    ...
    Velocity_encoder_LSTM = LSTM(hidden_units,return_sequences=True,dropout_U=0.1,
                        dropout_W=0.1,recurrent_initializer='zeros', bias_initializer='ones',return_state=True)
    Velocity_encoder_LSTM2 = LSTM(hidden_units,return_sequences=True,dropout_U=0.1,
                        dropout_W=0.1,recurrent_initializer='zeros', bias_initializer='ones',return_state=True)
    ...
    """___Channel__encoder___"""
    Channel_encoder_inputs = Input(shape=Channel_en_shape)
    Channel_encoder_LSTM = LSTM(hidden_units,return_sequences=True,dropout_U=0.1,
                        dropout_W=0.1,recurrent_initializer='zeros', bias_initializer='ones',return_state=True)
    Channel_encoder_LSTM2 = LSTM(hidden_units,return_sequences=True,dropout_U=0.1,
                        dropout_W=0.1,recurrent_initializer='zeros', bias_initializer='ones',return_state=True)
    Channel_encoder_outputs,_, _ = Channel_encoder_LSTM(Channel_encoder_inputs)
    Channel_encoder_outputs,Channel_state_h,Channel_state_c = Channel_encoder_LSTM2(Channel_encoder_outputs)
    Channel_encoder_states = [Channel_state_h, Channel_state_c]
    ...
    """___Channel__decoder___"""
    Channel_decoder_inputs = Input(shape=(None,Channel_de_shape[1]))
    Channel_decoder_LSTM = LSTM(hidden_units,dropout_U=0.2,dropout_W=0.2,return_sequences=True,
                        recurrent_initializer='zeros',bias_initializer='ones',return_state=True)
    Channel_decoder_LSTM2 = LSTM(hidden_units,dropout_U=0.2,dropout_W=0.2,return_sequences=True,
                        recurrent_initializer='zeros',bias_initializer='ones',return_state=True)
    Channel_decoder_outputs, _, _ = Channel_decoder_LSTM(Channel_decoder_inputs,initial_state=Channel_encoder_states)
    #Note_velo_decoder_outputs = Add()([Channel_decoder_outputs,Note_decoder_outputs])
    Channel_decoder_outputs, _, _ = Channel_decoder_LSTM2(Channel_decoder_outputs)
    Channel_decoder_dense = Dense(Channel_de_shape[1],activation='softmax',name="Channel_outputs")
    Channel_decoder_outputs = Channel_decoder_dense(Channel_decoder_outputs)
    ...
    losses={'note_outputs':'categorical_crossentropy',
            ...
            'time_outputs':'mse',
            'Channel_outputs':'categorical_crossentropy',}
    
    All_inputs = [Note_encoder_inputs,Type_encoder_inputs,Velocity_encoder_inputs,Time_encoder_inputs,Channel_encoder_inputs,
                  Note_decoder_inputs,Type_decoder_inputs,Velocity_decoder_inputs,Time_decoder_inputs,Channel_decoder_inputs]
    All_outputs = [Note_decoder_outputs,Type_decoder_outputs,Velocity_decoder_outputs,Time_decoder_outputs,Channel_decoder_outputs] 
    
    
    All_metrics = ['categorical_accuracy']
    ...
    x_channel_train,x_channel_test,y_channel_train,y_channel_test=tts(dataChannel['x'],dataChannel['y'],test_size=0.2)
    
    
    history= model.fit(x=[x_note_train,x_type_train,x_velocity_train,x_time_train,x_channel_train,
                          y_note_train,y_type_train,y_velocity_train,y_time_train,y_channel_train],
              y=[y_note_train,y_type_train,y_velocity_train,y_time_train,y_channel_train],
              batch_size=batch_size,
              epochs=epochs,
              verbose=1,
              validation_data=([x_note_test,x_type_test,x_velocity_test,x_time_test,x_channel_test,
                                y_note_test,y_type_test,y_velocity_test,y_time_test,y_channel_test],
                               [y_note_test,y_type_test,y_velocity_test,y_time_test,y_channel_test]))
    ...
    Channel_encoder_model_inf = Model(Channel_encoder_inputs,Channel_encoder_states)
    ...
    """___________________________________Channel____________________________________"""
    Channel_decoder_state_input_H = Input(shape=(hidden_units,))
    Channel_decoder_state_input_C = Input(shape=(hidden_units,)) 
    Channel_decoder_state_inputs = [Channel_decoder_state_input_H, Channel_decoder_state_input_C]
    Channel_decoder_outputs, _, _ = Channel_decoder_LSTM(Channel_decoder_inputs,
                                                                     initial_state=Channel_decoder_state_inputs)
    #Note_velo_decoder_outputs = Add()([Channel_decoder_outputs,Note_decoder_outputs])
    Channel_decoder_outputs, Channel_decoder_state_h, Channel_decoder_state_c = Channel_decoder_LSTM2(Channel_decoder_outputs)
    
    Channel_decoder_states = [Channel_decoder_state_h, Channel_decoder_state_c]
    Channel_decoder_outputs = Channel_decoder_dense(Channel_decoder_outputs)
    ...
    Channel_decoder_model_inf= Model([Channel_decoder_inputs]+Channel_decoder_state_inputs,
                         [Channel_decoder_outputs]+Channel_decoder_states)
    

    """"__________________________________________________________________________________"""
    
    scores = model.evaluate([x_note_test,x_type_test,x_velocity_test,x_time_test,x_channel_test,
                             y_note_test,y_type_test,y_velocity_test,y_time_test,y_channel_test],
                            [y_note_test,y_type_test,y_velocity_test,y_time_test,y_channel_test],
                            verbose=1)
    ...
    individual_models={'note':[Note_encoder_model_inf,Note_decoder_model_inf],
                       ...
                       'channel':[Channel_encoder_model_inf,Channel_decoder_model_inf]}
...
def composeSong(index):
    ...
    sample_song_Channel = np.reshape(generateSong(dataChannel['x'][index],Channel_en_shape,Channel_de_shape,
                                          AllModels['channel'][0],AllModels['channel'][1]),dataChannel['y'][index].shape)
    ...
    for k in range(len(completesong['time'])):
        track.append(Message(completesong['type'][k],
                             ...
                             channel=completesong['channel'][k]))
